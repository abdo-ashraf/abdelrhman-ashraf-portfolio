        ...
        <div className="max-w-3xl mx-auto">
          <ScrollReveal delay={0.1}>
            <div className="grid md:grid-cols-2 gap-6 mb-12">
              ...
              const content = <div className="flex items-center gap-4 p-6 bg-card rounded-xl shadow-card hover:shadow-card-hover transition-all duration-300 group h-full">
              ...
              return info.href ? <a key={info.label} href={info.href} target={info.href.startsWith('http') ? '_blank' : undefined} rel={info.href.startsWith('http') ? 'noopener noreferrer' : undefined} className="block h-full">
                    {content}
                  </a> : <div key={info.label} className="h-full">{content}</div>;
            })}
            </div>
          </ScrollReveal>
